...
### Manual copy from old `icloudpd-rs` paths

Current kei releases don't auto-copy the old `icloudpd-rs` config, cookies, or
state DB at startup. If you still have files in those paths, copy only the
ones you need before the first run:
...
Run only the `cp` commands whose source paths exist. Leave the old files in
place until you've confirmed kei starts and can read the copied state.
...
offset to sidecars and to embedded files that do not already hold a timestamp.
To replace an older embedded timestamp, enable `metadata.set_exif_datetime`
and run `kei sync --refresh-metadata --repair-capture-timestamps`. This can
overwrite camera-supplied metadata.
