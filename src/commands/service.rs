...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn init_photos_service_capture_survives_421_and_provider_operations() {
        use futures_util::StreamExt;
        use icloud::photos::capture::ResponseCapture;
        use icloud::photos::{ProviderRecordId, RecordLookupRequest, RecordResolution};
        ...
        let misdirected = "  misdirected request\n";
        let indexing = r#" {"records":[{"recordName":"CheckIndexingState","fields":{"state":{"value":"FINISHED"}}}]} "#;
        let private = r#" {"zones":[]} "#;
        let shared = r#" {"zones":[{"zoneID":{"zoneName":"SharedSync-capture"}}]} "#;
        let photos = r#" {"records":[
            {"recordName":"master","recordType":"CPLMaster","fields":{}},
            {"recordName":"asset","recordType":"CPLAsset","fields":{
                "masterRef":{"value":{"recordName":"master"}},
                "assetDate":{"value":1700000000000}
            }}],"syncToken":"full","unknown":1,"unknown":2} "#;
        let tail = " {\"records\":[],\"syncToken\":\"full\"} \n";
        let database = r#" {"zones":[],"syncToken":"database","moreComing":false} "#;
        let changes = r#" {"zones":[{"zoneID":{"zoneName":"SharedSync-capture"},"records":[],"syncToken":"delta","moreComing":false}]} "#;
        let lookup = r#" {"records":[{"recordName":"missing","serverErrorCode":"UNKNOWN_ITEM"}]} "#;
        ...
        Mock::given(method("POST"))
            ...
            .await;
        Mock::given(method("POST"))
            .and(path_regex(r"/records/query$"))
            .and(body_partial_json(serde_json::json!({
                "query": {"recordType": "CheckIndexingState"}
            })))
            ...
            .await;
        for (path, body) in [
            (r"/private/zones/list$", private),
            (r"/shared/zones/list$", shared),
            (r"/private/changes/database$", database),
            (r"/shared/changes/zone$", changes),
            (r"/shared/records/lookup$", lookup),
        ] {
            Mock::given(method("POST"))
                .and(path_regex(path))
        ...
        for (body, count) in [(photos, 1), (tail, icloud::photos::MAX_EMPTY_PAGE_PROBES)] {
            Mock::given(method("POST"))
                .and(path_regex(r"/shared/records/query$"))
                .and(body_partial_json(serde_json::json!({
                    "query": {"recordType": "CPLAssetAndMasterByAssetDateWithoutHiddenOrDeleted"}
                })))
                .respond_with(ResponseTemplate::new(200).set_body_string(body))
                .up_to_n_times(u64::from(count))
                .expect(u64::from(count))
                .mount(&server)
                .await;
        }

        let (session, mut service) = init_photos_service(
            fake_auth_result(&server.uri()).await,
            no_delay_retry(),
            crate::personality::Mode::Off,
        ...
        let album = libraries
            .iter()
            .find(|library| library.zone_name() == "SharedSync-capture")
            .unwrap()
            .all();
        let assets = album.photos(None).await.unwrap();
        assert_eq!(assets.len(), 1);
        assert_eq!(assets[0].asset_record_name(), "asset");
        assert_eq!(
            service.changes_database(None).await.unwrap().sync_token,
            "database"
        );
        let (mut stream, token) = album.changes_stream("full");
        assert!(stream.next().await.is_none());
        assert_eq!(token.await.unwrap(), "delta");
        let result = album
        ...
        assert!(matches!(
            ...
            [(id, RecordResolution::Deleted { master_family: false, .. })]
                if id.as_str() == "missing"
        ));
        drop((stream, album, libraries, service, session));
        capture.finish().await.unwrap();

        let mut expected = vec![misdirected, indexing, private, shared, indexing, photos];
        expected.extend(std::iter::repeat_n(
            tail,
            icloud::photos::MAX_EMPTY_PAGE_PROBES as usize,
        ));
        expected.extend([database, changes, lookup]);
        let requests = server.received_requests().await.unwrap();
        assert_eq!(requests.len(), expected.len());
        let runs = std::fs::read_dir(data.path().join(".diagnostics"))
            .unwrap()
            .collect::<Result<Vec<_>, _>>()
            .unwrap();
        assert_eq!(runs.len(), 1);
        let run = runs[0].path();
        assert_eq!(std::fs::read_dir(&run).unwrap().count(), requests.len());
        for (index, body) in expected.iter().enumerate() {
            assert_eq!(
                std::fs::read(run.join(format!("{:06}.body", index + 1))).unwrap(),
                body.as_bytes()
            );
        }
        server.verify().await;
