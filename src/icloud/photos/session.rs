...
mod wiremock_tests {
    ...
    use super::{
        CapturingPhotosSession, HttpStatusError, PhotosSession, classify_api_error, retry_post,
    };
    use crate::icloud::photos::capture::ResponseCapture;
    use crate::retry::{RetryAction, RetryConfig};

    async fn session(data: &Path) -> CapturingPhotosSession {
        ...
        .await
        .unwrap();
        CapturingPhotosSession {
            session: Arc::new(tokio::sync::RwLock::new(auth)),
            capture: ResponseCapture::new(data).await.unwrap(),
        }
    }

    fn run_dir(data: &Path) -> PathBuf {
    ...
    #[tokio::test]
    async fn capture_redirect_bodies_and_post_method_semantics() {
        let server = crate::start_wiremock_or_skip!();
        for status in [301, 302, 303, 307, 308] {
            server.reset().await;
            let data = tempfile::tempdir().unwrap();
            let session = session(data.path()).await;
            let intermediate = b"redirect body\n".repeat(8192);
            let final_body = b" {\"records\":[]} \n";
            Mock::given(path("/start"))
                .respond_with(
                    ResponseTemplate::new(status)
                        .insert_header("Location", "final")
                        .insert_header("Set-Cookie", "redirect=shared; Path=/")
                        .set_body_bytes(intermediate.clone()),
                )
                .mount(&server)
                .await;
            let run = run_dir(data.path());
            let saved = run.join("000001.body");
            let expected = intermediate.clone();
            Mock::given(path("/final"))
                .respond_with(move |_request: &wiremock::Request| {
                    assert_eq!(std::fs::read(&saved).unwrap(), expected);
                    ResponseTemplate::new(200).set_body_bytes(final_body.to_vec())
                })
                .mount(&server)
                .await;
            session
            ...
            assert_eq!(initial.method, "POST");
            assert_eq!(initial.body, b"{}");
            if status <= 303 {
                ...
                assert_eq!(redirected.body, b"{}");
                assert_eq!(redirected.headers["content-type"], "text/plain");
            }
            assert_eq!(redirected.headers["cookie"], "redirect=shared");
            assert_eq!(
                std::fs::read(run.join("000001.body")).unwrap(),
                intermediate
            );
            assert_eq!(std::fs::read(run.join("000002.body")).unwrap(), final_body);
            assert_eq!(std::fs::read_dir(run).unwrap().count(), 2);
        }
    }

    #[tokio::test]
    async fn capture_rejects_redirect_escape_and_invalid_location_without_retry() {
        let server = crate::start_wiremock_or_skip!();
        let outside = crate::start_wiremock_or_skip!();
        ...
        for location in [
            format!("{}/auth?secret=private", outside.uri()),
            "http://[invalid?secret=private".into(),
            server.uri().replacen("http://", "http://private@", 1),
        ] {
            server.reset().await;
            let data = tempfile::tempdir().unwrap();
            let session = session(data.path()).await;
            Mock::given(method("POST"))
                ...
                .mount(&server)
                .await;
            let error = retry_post(&session, &server.uri(), "{}", &[], &config)
                .await
                .unwrap_err();
            assert_eq!(classify_api_error(&error), RetryAction::Abort);
            assert!(!format!("{error:#}").contains("private"));
            assert_eq!(server.received_requests().await.unwrap().len(), 1);
            assert!(outside.received_requests().await.unwrap().is_empty());
            assert_eq!(
                std::fs::read(run_dir(data.path()).join("000001.body")).unwrap(),
                b"redirect"
            );
            session.capture.finish().await.unwrap();
        }
    ...
    #[tokio::test]
    async fn capture_redirect_limit_saves_last_response_and_aborts_without_retry() {
        let server = crate::start_wiremock_or_skip!();
        let data = tempfile::tempdir().unwrap();
        let session = session(data.path()).await;
        Mock::given(method("POST"))
        ...
        let run = run_dir(data.path());
        assert_eq!(std::fs::read_dir(&run).unwrap().count(), 11);
        for number in 1..=11 {
            assert_eq!(
                std::fs::read(run.join(format!("{number:06}.body"))).unwrap(),
                b"loop"
            );
        }
        session.capture.finish().await.unwrap();
    ...
    #[tokio::test]
    async fn capture_clone_uses_replacement_auth_client() {
        let server = crate::start_wiremock_or_skip!();
        Mock::given(method("POST"))
        ...
        let data = tempfile::tempdir().unwrap();
        let session = session(data.path()).await;
        let clone = session.clone_box();
        clone.post(&server.uri(), "{}".into(), &[]).await.unwrap();
        let mut auth = session.session.write().await;
        auth.release_lock().unwrap();
        *auth = crate::auth::session::Session::new(
            data.path(),
            "capture@example.test",
        ...
        let requests = server.received_requests().await.unwrap();
        assert_eq!(requests.len(), 2);
        assert_eq!(requests[0].headers["origin"], "https://example.test");
        assert_eq!(
            requests[1].headers["origin"],
            "https://replacement.example.test"
        );
        assert_eq!(std::fs::read_dir(run_dir(data.path())).unwrap().count(), 2);
        session.capture.finish().await.unwrap();
    }

    #[tokio::test]
    async fn capture_exact_success_and_bounded_error_bodies() {
        let server = crate::start_wiremock_or_skip!();
        let data = tempfile::tempdir().unwrap();
        let session = session(data.path()).await;
        let raw = b" {\"records\":[],\"unknown\":1,\"unknown\":2} \n";
        Mock::given(method("POST"))
            .respond_with(ResponseTemplate::new(200).set_body_bytes(raw.to_vec()))
            .mount(&server)
            .await;
        assert_eq!(
            session.post(&server.uri(), "{}".into(), &[]).await.unwrap()["unknown"],
            2
        );
        assert_eq!(
            std::fs::read(run_dir(data.path()).join("000001.body")).unwrap(),
        ...
        server.reset().await;
        let raw = b"no auth method found ".repeat(4096);
        Mock::given(method("POST"))
            .respond_with(ResponseTemplate::new(401).set_body_bytes(raw.clone()))
            .mount(&server)
            .await;
        let error = session
            .post(
                &format!("{}/?secret=private", server.uri()),
                "{}".into(),
                &[],
            )
            .await
            .unwrap_err();
        let http = error.downcast_ref::<HttpStatusError>().unwrap();
        assert!(
            http.body
                .as_ref()
                .unwrap()
                .starts_with("no auth method found")
        );
        assert!(http.body.as_ref().unwrap().len() <= 1027);
        assert!(!format!("{error:#}").contains("private"));
        assert!(!format!("{error:#}").contains("no auth"));
        assert_eq!(
            std::fs::read(run_dir(data.path()).join("000002.body")).unwrap(),
            raw
        );
        session.capture.finish().await.unwrap();
    }
    ...
    #[tokio::test]
    async fn capture_retries_invalid_json_empty_utf8_and_429_without_latching() {
        let server = crate::start_wiremock_or_skip!();
        let config = RetryConfig {
            max_retries: 2,
            base_delay_secs: 0,
        ...
        for (status, raw) in [
            ...
            (200, Vec::new()),
            (200, vec![0xff, 0xfe]),
            (429, b"throttled".to_vec()),
        ] {
            server.reset().await;
            Mock::given(method("POST"))
                .respond_with(ResponseTemplate::new(status).set_body_bytes(raw.clone()))
                .mount(&server)
                .await;
            let data = tempfile::tempdir().unwrap();
            let session = session(data.path()).await;
            let error = retry_post(&session, &server.uri(), "{}", &[], &config)
                .await
                .unwrap_err();
            assert_eq!(classify_api_error(&error), RetryAction::Retry);
            assert_eq!(server.received_requests().await.unwrap().len(), 3);
            for number in 1..=3 {
                assert_eq!(
                    std::fs::read(run_dir(data.path()).join(format!("{number:06}.body"))).unwrap(),
                    raw
                );
            }
            assert_eq!(std::fs::read_dir(run_dir(data.path())).unwrap().count(), 3);
            session.capture.finish().await.unwrap();
            let client = reqwest::Client::new();
            retry_post(&client, &server.uri(), "{}", &[], &config)
                .await
                .unwrap_err();
            assert_eq!(
                server.received_requests().await.unwrap().len(),
                6,
                "capture must preserve attempt count"
            );
        }
    }

    #[tokio::test]
    async fn capture_disabled_and_sticky_write_failure_send_no_extra_requests() {
        let server = crate::start_wiremock_or_skip!();
        Mock::given(method("POST"))
            .respond_with(ResponseTemplate::new(200).set_body_string("{}"))
            .mount(&server)
        ...
        PhotosSession::post(&reqwest::Client::new(), &server.uri(), "{}".into(), &[])
            .await
            .unwrap();
        assert!(!data.path().join(".diagnostics").exists());
        let session = session(data.path()).await;
        std::fs::write(run_dir(data.path()).join("000001.body.part"), b"keep").unwrap();
        let config = RetryConfig {
            max_retries: 3,
            base_delay_secs: 0,
            max_delay_secs: 0,
        ...
        assert!(
            session
                .clone_box()
                .post(&server.uri(), "{}".into(), &[])
                .await
                .is_err()
        );
        assert_eq!(server.received_requests().await.unwrap().len(), 2);
        assert!(session.capture.finish().await.is_err());
        assert_eq!(
            std::fs::read(run_dir(data.path()).join("000001.body.part")).unwrap(),
            b"keep"
        );
    }

    #[tokio::test]
    async fn capture_clones_run_concurrently_and_finish_closes_admission() {
        let server = crate::start_wiremock_or_skip!();
        Mock::given(method("POST"))
            .respond_with(
        ...
        .await
        .unwrap();
        // A live HTTP call must not retain the authentication lock.
        let auth_lock = session.session.try_write().unwrap();
        drop(auth_lock);
        session.capture.finish().await.unwrap();
        for task in tasks {
            assert_eq!(task.await.unwrap().unwrap(), serde_json::json!({}));
        }
        assert_eq!(std::fs::read_dir(run_dir(data.path())).unwrap().count(), 4);
        assert!(session.post(&server.uri(), "{}".into(), &[]).await.is_err());
        assert_eq!(server.received_requests().await.unwrap().len(), 4);
    }

    #[tokio::test]
    async fn capture_retries_truncated_transport_body_but_finish_reports_incomplete() {
        if crate::test_helpers::skip_if_loopback_bind_blocked("capture_transport_retry") {
            return;
        }
        ...
        let data = tempfile::tempdir().unwrap();
        let session = session(data.path()).await;
        let part = run_dir(data.path()).join("000001.body.part");
        let server = tokio::spawn(async move {
            ...
            for truncated in [true, false] {
                ...
                if truncated {
                    ...
                    // Ensure the prefix has been consumed before forcing EOF.
        ...
        session.capture.check().unwrap();
        assert!(session.capture.finish().await.is_err());
        let run = run_dir(data.path());
        assert_eq!(
            std::fs::read(run.join("000001.body.part")).unwrap(),
            b"{partial"
        );
        assert_eq!(std::fs::read(run.join("000002.body")).unwrap(), b"{}");
        assert_eq!(std::fs::read_dir(run).unwrap().count(), 2);
    }

    #[tokio::test]
    async fn capture_body_transport_failure_and_cancellation_leave_partial() {
        if crate::test_helpers::skip_if_loopback_bind_blocked("capture_partial") {
            return;
        }
        for cancel in [false, true] {
            let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
            let url = format!("http://{}/?secret=private", listener.local_addr().unwrap());
            ...
            let session = session(data.path()).await;
            let clone = session.clone_box();
            let task = tokio::spawn(async move { clone.post(&url, "{}".into(), &[]).await });
            let part = run_dir(data.path()).join("000001.body.part");
            tokio::time::timeout(Duration::from_secs(5), async {
                while tokio::fs::metadata(&part)
                    .await
            ...
            server.await.unwrap();
            assert_eq!(session.capture.check().is_err(), cancel);
            assert!(session.capture.finish().await.is_err());
            assert_eq!(std::fs::read(part).unwrap(), b"{partial");
            assert!(!run_dir(data.path()).join("000001.body").exists());
        }
    }
}
