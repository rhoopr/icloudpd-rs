...
/// Stub implementation of StateDb for testing.
#[cfg(test)]
pub struct StubStateDb {
    pub should_download_result: std::sync::atomic::AtomicBool,
}

#[cfg(test)]
impl StubStateDb {
    pub fn new(should_download: bool) -> Self {
        Self {
            should_download_result: std::sync::atomic::AtomicBool::new(should_download),
        }
    }
}
...
impl StateDb for StubStateDb {
    async fn should_download(
        ...
        Ok(self
            .should_download_result
            .load(std::sync::atomic::Ordering::SeqCst))
    }

    async fn upsert_seen(&self, _record: &AssetRecord) -> Result<(), StateError> {
        Ok(())
    }
