...
- Embedded XMP writes for HEIC, HEIF, and AVIF files now use a byte-preserving item-map writer. Multi-image files such as HDR and portrait captures carry an XMP packet per image, so kei writes the one describing the primary image and leaves gain map, depth, and matte metadata untouched. A packet added to an HDR capture describes its tone-mapped image alongside the primary, so the file keeps its HDR rendition. Unsupported layouts fail the metadata write without replacing the media file. ([#558])
