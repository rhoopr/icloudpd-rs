...
- **`KEI_WATCH_WITH_INTERVAL=` (empty value) is now treated as unset.** The docker image bakes `ENV KEI_WATCH_WITH_INTERVAL=86400` so `docker run kei sync ...` defaults to a 24h watch loop. Previously, passing `-e KEI_WATCH_WITH_INTERVAL=` to override the image default for a one-shot invocation failed with `KEI_WATCH_WITH_INTERVAL is not a valid integer ("")` because the env parser tried to parse the empty string. Empty value now matches conventional shell semantics (treated as unset) and lets users force a one-shot from the published image without rebuilding it.
