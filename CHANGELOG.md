...
- **`kei import-existing` summary lists `Filtered (no path)` and `Hash errors`.** Previously the completion banner reported only `Total assets scanned`, `Files matched`, and `Unmatched versions` - any asset dropped before path derivation (no versions, content/date filter, `--live-photo-mode skip`, `--force-size` with a missing size) or any file whose SHA256 read failed (permissions, vanished mid-scan, I/O error) just disappeared from the tally. Operators can now reconcile against the on-disk tree: an empty `Filtered` line means every enumerated asset reached path derivation, and an empty `Hash errors` line means every matched file was hashable. Empty-expected-paths now also emits a `WARN` line per asset naming the id and the relevant config.
