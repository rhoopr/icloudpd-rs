...
- **Terminal Apple auth failures get their own exit path.** Apple `/signin/complete` responses with `serviceErrors[].code = "-20209"` now surface as a terminal auth error with iforgot.apple.com and stored-password recovery steps, and kei exits 4 instead of the generic auth exit. ([#333])
