...
- **Opt-in `--friendly` progress UI.** Set `--friendly` (or `KEI_FRIENDLY=1`) to swap the progress bar for a smoothed multi-pass card: source-aware top rule (`kei · downloading from iCloud`, dynamic when more backends land), bandwidth sparkline, smart ETA, album-or-unfiled prefix on the active filename, color-tiered rules (bright cyan under 30%, cyan to 70%, green past), moon-phase spinner, and a stripped tracing format without the target+timestamp prefix. Bandwidth (B/s, KB/s, MB/s, GB/s) replaces items/sec. Off by default; force-off when stderr or stdout isn't a TTY, when `--no-progress-bar` / `--only-print-filenames` / `--report-json` is set, when running under `kei service run` / inside Docker / under systemd, when `TERM=dumb`, or when `--log-level` / `RUST_LOG` is set explicitly. v0.13 output is byte-for-byte unchanged when the flag isn't passed.
- **`--verbose` / `-v` shorthand.** Aliases `--log-level info` and restores the structured `target+timestamp` log format friendly mode hides.
- **Friendly-mode lifecycle narration and verb cycling.** While `--friendly` is on, the bar's pre-first-file `scanning...` line cycles through a phase-aware verb pool ("looking around", "counting albums", "asking iCloud what is new") every ~600ms so the listing/scan gap doesn't read as a stall. Smart-ETA wording escalates from `calculating...` to `still calculating...` after 5 seconds of indeterminate ETA. Sync runs print four narration lines alongside the bar: a greeting before the first cycle, a `Signed in as <user>.` confirmation after auth, a `Found N libraries to sync.` line after library resolution, and a per-cycle sign-off (`Done. 12 new files in 3 minutes.`). All four lines and the verb cycling are friendly-mode-only; off mode is unchanged.
- **Friendly-mode watch heartbeat and graceful-exit closer.** Between cycles in `--friendly` watch mode, kei prints `Sleeping until 14:32. Press Ctrl+C to stop.` (24h local clock) so an idle process reads as deliberately waiting rather than hung. On a graceful Ctrl+C exit, after in-flight downloads drain, it prints `Done. See you next time.` Off mode is unchanged: the existing `tracing::info!(interval_secs)` "Waiting before next cycle" and shutdown lines still fire for journals.
- **Friendly-mode narration around CloudKit 421 recovery.** When `init_photos_service` hits a 421 Misdirected Request and resets the HTTP/2 connection pool, `--friendly` users see `iCloud connection wobbled. Resetting...` before the retry and `Back on track.` once it succeeds, so the brief stall reads as a known recovering condition rather than the bar locking up. The existing `tracing::warn!` events with the partition URL still fire for off-mode journal consumers. `kei list` and `kei import-existing` pass `Mode::Off` (no friendly bar to coexist with), preserving today's exact output.
- **Friendly-mode narration around per-file download retries.** When a download hits a retryable error (rate-limit, 5xx, transient network failure) and the pipeline is about to back off, `--friendly` now prints `iCloud hiccup. Retrying in 4s...` so the user sees the pause is intentional. On recovery: `Back on track.` After the final attempt is exhausted: `That one is being stubborn. Skipping for now, will retry next sync.` Sub-second delays render as `1s` so the line never reads as "no pause". The bookend lines fire only when at least one pause was narrated, so one-shot failures stay silent. CloudKit API retries (album listing, query, sync-token paging) explicitly opt out: those resolve in well under a second and would flicker noisily. The structured `tracing::warn!` event still fires for off-mode journals.
- **Friendly-mode 2FA prompt context line.** When 2FA is required during a `--friendly` sync, kei prints `Sent a code to your trusted devices. Approve the push and enter the 6-digit code below.` above the existing `Enter 2FA code (or press Enter to request a new code):` prompt. Off mode keeps the bare prompt for scripted consumers (`kei login submit-code`, etc.). On a wrong-code retry the friendly line re-prints each iteration.
- **Friendly mode is now on by default on plain TTYs.** First contact with `kei sync` on an interactive terminal already gets the verb-cycling spinners, summary card, sign-off line, and lifecycle narration without touching a flag. The auto-off rules from the opt-in surface carry over unchanged: non-TTY stdio, `--report-json`, `--only-print-filenames`, `--no-progress-bar`, `kei service run`, container / systemd contexts, `TERM=dumb`, and an explicit `--log-level` / `RUST_LOG` all still force off, so journals and pipes get the v0.13 structured format byte-for-byte. Two new escape hatches: `--no-friendly` overrides per invocation, and `[ui] friendly = false` in `config.toml` persists the opt-out. Resolution chain is CLI > TOML > default-on, with the hard-off contexts winning over the lot.
- **`kei config setup` asks "Show friendly progress messages?".** One Y/n question slots in after the watch-mode step. A "yes" answer leaves the section commented so the runtime keeps freedom to evolve the default later; a "no" answer persists `[ui] friendly = false`. Skipping the wizard or the question falls through to the default-on-for-TTY policy.
...
- **`kei import-existing` accepts repeatable `--library` and the deprecated TOML album keys.** `--library` was single-value before, so a second `--library` flag silently overrode the first. It now takes the same repeatable v0.13 grammar as `kei sync --library` (`primary` / `shared` / `all` / raw zone names / `!name` exclusions). Deprecated TOML keys `[filters].album` (singular) and `[filters].exclude_albums` are now read on the import path with the same deprecation warning sync emits, so a config mid-migration produces the same scope for both commands instead of letting import fall back to every album. ([#215])
