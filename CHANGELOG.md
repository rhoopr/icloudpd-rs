...
- **`KEI_WATCH_WITH_INTERVAL=` (empty value) is now treated as unset.** Lets users force a one-shot from the published docker image with `docker run -e KEI_WATCH_WITH_INTERVAL= ... kei sync ...`; previously the parser rejected the empty string and there was no other way to disable the image's baked-in 24h watch default without rebuilding.
